...
function MorePostsFromUser({ postId }) {
  ...
  const [getMorePostsFromUser, { data: morePosts, loading: loadingMorePosts }] = useLazyQuery(GET_MORE_POSTS_FROM_USER);
    ...
    <div className={classes.container}>
    {
      ...
      : (
          <>
            ...
            >
              More Posts from{" "}
              <Link to={`/${data.posts_by_pk.user.username}`} className={classes.link}>@{data.posts_by_pk.user.username}</Link>
            </Typography>
